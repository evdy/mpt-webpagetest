<?php
	...
	$SQLString = "SELECT * FROM t_page_load_report_pi where product_id = " . $aPID;
    ...
    function get_query_results_report_pi($aLink, $aPID, $aInstID) {
	if ($aInstID == 1) {
	    $SQLString = "SELECT * FROM t_page_load_report where product_id = " . $aPID;
	}
	else if ($aInstID == 2){
	    $SQLString = "SELECT * FROM t_page_load_report_pi where product_id = " . $aPID;
	}
